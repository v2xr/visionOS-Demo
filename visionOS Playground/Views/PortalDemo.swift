...
struct PortalDemo: View {
  ...
  func makeWorld() -> Entity {
    let world = Entity()
    world.components.set(ModelComponent(mesh: .generateSphere(radius: 0.1), materials: [OcclusionMaterial()]))
...
//    world.addChild(ufo)
  ...
  func makePortal(world: Entity) -> Entity {
    let portal = Entity()
    portal.components.set(ModelComponent(mesh: .generatePlane(width: 0.2, height: 0.2, cornerRadius: 0.1), materials: [PortalMaterial()]))
