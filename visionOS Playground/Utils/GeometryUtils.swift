...
extension GeometrySource {
  func asArray<T>(ofType _: T.Type) -> [T] {
  ...
  func asSIMD3<T>(ofType _: T.Type) -> [SIMD3<T>] {
...
extension simd_float4x4 {
  ...
  var translation: SIMD3<Float> {
    ...
    set {
      columns.3 = [newValue.x, newValue.y, newValue.z, 1]
    }
  }
...
extension MeshResource.Contents {
  init(planeGeometry: PlaneAnchor.Geometry) {
    self.init()
    instances = [MeshResource.Instance(id: "main", model: "model")]
    ...
    models = [MeshResource.Model(id: "model", parts: [part])]
  }
}
