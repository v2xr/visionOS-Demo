...
public struct GestureComponent: Component, Codable {
  ...
  private mutating func handlePivotDrag(value: EntityTargetValue<DragGesture.Value>) {
    let state = EntityGestureState.shared
    ...
    if let inputDevicePose = value.inputDevicePose3D {
      ...
      targetPivotTransform.translation = value.convert(
        // MARK: Fix Y Axis
        inputDevicePose.position,
        from: .local, to: .scene
      )
  ...
  private mutating func handleFixedDrag(value: EntityTargetValue<DragGesture.Value>) {
    let state = EntityGestureState.shared
    guard let entity = state.targetedEntity else {
      fatalError("Gesture contained no entity")
    }
  ...
  mutating func onEnded(value _: EntityTargetValue<DragGesture.Value>) {
    let state = EntityGestureState.shared
  ...
  mutating func onChanged(value: EntityTargetValue<MagnifyGesture.Value>) {
    let state = EntityGestureState.shared
    guard canScale, !state.isDragging else {
      return
    }
  ...
  mutating func onEnded(value _: EntityTargetValue<MagnifyGesture.Value>) {
  ...
  mutating func onChanged(value: EntityTargetValue<RotateGesture3D.Value>) {
    let state = EntityGestureState.shared
    guard canRotate, !state.isDragging else {
      return
    }
    ...
    let flippedRotation = Rotation3D(angle: rotation.angle,
                                     axis: RotationAxis3D(x: 0, // -rotation.axis.x,
                                                          y: rotation.axis.y,
                                                          z: 0)) // -rotation.axis.z))
  ...
  /// Handle `.onChanged` actions for rotate  gestures.
  mutating func onEnded(value _: EntityTargetValue<RotateGesture3D.Value>) {
