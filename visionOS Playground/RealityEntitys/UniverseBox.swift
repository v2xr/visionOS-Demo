...
class UniverseBox: Entity {
  ...
  init(data: UniverseData, color: Color = .white) {
    ...
    back.components.set(PhysicsBodyComponent(shapes: [backShape], mass: 1.0, mode: .static))
    faces[.back] = back
    ...
    front.components.set(PhysicsBodyComponent(shapes: [backShape], mass: 1.0, mode: .static))
    ...
    left.components.set(PhysicsBodyComponent(shapes: [leftRightShape], mass: 1.0, mode: .static))
    faces[.left] = left
    ...
    right.components.set(PhysicsBodyComponent(shapes: [leftRightShape], mass: 1.0, mode: .static))
    faces[.right] = right
    ...
    top.components.set(PhysicsBodyComponent(shapes: [topBottomShape], mass: 1.0, mode: .static))
    ...
    bottom.components.set(PhysicsBodyComponent(shapes: [topBottomShape], mass: 1.0, mode: .static))
  ...
  func spawnAt(position: SIMD3<Float> = [0, 0, 0]) {
    ball = ModelEntity(mesh: .generateSphere(radius: ballSize), materials: [UnlitMaterial(color: .init(color))])
    ball!.name = "Ball"
    ball!.components.set(CollisionComponent(shapes: [.generateSphere(radius: ballSize)], collisionOptions: [.fullContactInformation]))
    var physic: PhysicsBodyComponent = .init(shapes: [.generateSphere(radius: ballSize)], mass: 0.05, material: .generate(friction: 0.6), mode: .static)
